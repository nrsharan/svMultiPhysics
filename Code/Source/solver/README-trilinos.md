...
- `trilinos-frosch-block`: `trilinos-frosch` with two blocks of degrees of freedom, each with its own coarse space: the first `nsd` degrees of freedom of every node (displacements, whose coarse space also contains the rotations computed from the nodal coordinates) and the remaining ones (e.g. the concentration of the `deformation-diffusion` equation), as FEDDLib does for the structure-chemistry problem. FROSch's block preconditioner needs block-contiguous degree-of-freedom numbers, so it works on a renumbered copy of the matrix (all renumbered degrees of freedom stay on their process). Needs more degrees of freedom per node than space dimensions.
...
For the `deformation-diffusion` equation, the fastest setup measured on `tests/cases/def_diffu/artery_dan` (16 and 32 processes, see its README) is `trilinos-frosch-block` with the FROSch settings of FEDDLib (`tests/cases/def_diffu/artery_dan/frosch_block_feddlib.xml`: coarse basis entries below 1e-5 dropped, coarse basis of the first matrix kept); the default `trilinos-frosch` is within about 10% of it on 32 processes. FROSch's default (true, also used by FEDDLib) keeps the coarse basis of the first matrix for all later ones; `tests/cases/def_diffu/artery_dan/frosch_recycle_coarse_basis.xml` is a parameter file doing so.

At the end of a run with `trilinos-frosch` or `trilinos-frosch-block`, the Teuchos timers are printed (as FEDDLib does), summed over the run: FROSch's `initialize`, `compute` (split into the overlapping and the coarse operator) and `apply`, Belos's solve, and `svMP Trilinos: create graph and matrix`, `preconditioner setup` and `solve (total)`. The rest of the run time is mostly the assembly.
